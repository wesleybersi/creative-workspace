...
const PlotGrid: React.FC<Props> = ({ collage }) => {
  ...
  const { newSection, client, isMouseDown, mode } = useStore();
  const { size } = collage;
  const {
    set,
    selection,
  ...
  useEffect(() => {
    if (!isMouseDown && selection && selectedType) {
      newSection(client.collages.indexOf(collage), selectedType, selection);
      set({ selection: null });
    }
  }, [isMouseDown]);
  ...
  useEffect(() => {
    ...
    for (const section of collage.sections) {
      const firstIndex =
        section.position.row.start * collage.size.rows +
  ...
  }, [gridRef, collage.sections, collage.sections.length]);
